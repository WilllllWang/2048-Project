...
void drawGame(Board *board, Blank *blank, ALLEGRO_FONT *gameFont, ALLEGRO_BITMAP  *gameBackground, ALLEGRO_DISPLAY *display, ALLEGRO_FONT *countFont) {
    al_draw_bitmap(gameBackground, 0, 0, 0);//繪製遊戲背景
    al_draw_textf(gameFont, al_map_rgb(0, 0, 0), SCREEN_WIDTH / 2, (SCREEN_HEIGHT / 2) - 330, ALLEGRO_ALIGN_CENTRE, "Score: %d", board->score);//繪製分數
    ...
    al_draw_textf(countFont, al_map_rgb(0, 0, 0), SCREEN_WIDTH / 3+425, (SCREEN_HEIGHT / 2) - 450, ALLEGRO_ALIGN_CENTRE, "2048: %d", blank->blank2048);// 繪製數字方塊計數

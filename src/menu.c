...
void menu(ALLEGRO_BITMAP  *startButton,  ALLEGRO_BITMAP  *menuBackground,  ALLEGRO_MOUSE_STATE Mstate, ALLEGRO_DISPLAY *display, ALLEGRO_FONT *font) {
    ...
    while (1) {
        ...
        if ((mouseX >= startButtonValue.startButtonX && mouseX <= (startButtonValue.startButtonX+startButtonValue.startButtonWidth)) && (mouseY >= startButtonValue.startButtonY && mouseY <= (startButtonValue.startButtonY+startButtonValue.startButtonHeight))) {
            al_draw_text(font, al_map_rgb(255, 255, 255), SCREEN_WIDTH/2, startButtonValue.startButtonY+15, ALLEGRO_ALIGN_CENTER, "PLAY"); 
            mouseOnButton = true;
        }
        else {
            al_draw_text(font, al_map_rgb(220, 110, 90), SCREEN_WIDTH/2, startButtonValue.startButtonY+15, ALLEGRO_ALIGN_CENTER, "PLAY");
            mouseOnButton = false;
        }
        al_flip_display();  
